...
export class NotificationService {
    constructor() {
        ...
        this.source.addEventListener('newBoard', (e) => this.handleNewBoard(e));
        this.source.addEventListener('newPath', (e) => this.handleNewPath(e));
        this.source.onerror = (e) => this.handleError(e);
    }
    handleNewBoard(e) {
        const data = JSON.parse(e.data);
        this.showToast(`User ${data.creator_username} has created a new board: “${data.board_name}”`, `/polls/boards/${data.board_id}/`);
    }
    handleNewPath(e) {
        const data = JSON.parse(e.data);
        this.showToast(`User ${data.user_username} has saved path on the board: “${data.board_name}”`, `/polls/boards/${data.board_id}/draw/`);
    }
    handleError(e) {
